...
class WLDevice {
  ...
  _open() {
    try {
      this.dev = new HID.HID(this.info.path);
    } catch (err) {
      const msg = String(err.message || err);
      if (/privilege violation|not permitted|access/i.test(msg)) {
        throw new PermissionError(
          "macOS denied access to the Work Louder HID interface. Grant " +
            "Input Monitoring to the process running this plugin under " +
